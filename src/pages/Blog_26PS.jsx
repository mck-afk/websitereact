import React from "react";
import PageLayout from "../components/PageLayout";
import { useNavigate } from "react-router-dom";

const Blog_26PS = () => {
    const navigate = useNavigate();
    return (
        <PageLayout>
            <div className="red" style={{ paddingBottom: "5px" }}>
                <h1 className="purple">2026+ Predictions</h1>
                <h2>astrology & technology forecast</h2>
                <h4>13 January 2026 | astrology, technology, predictions</h4>
                <button className="red" onClick={() => navigate(-1)}>back</button>
            </div>
            <p className="red">Updates will be dated and highlighted in Red</p>
            <h2>Most valuable commodity for the next 10 years = Physical Presence</h2>
            <p>The <a href="https://en.wikipedia.org/wiki/Attention_economy">age of attention</a> will decay. Digital spaces are <a href="https://www.theguardian.com/news/ng-interactive/2026/jan/11/how-grok-nudification-tool-went-viral-x-elon-musk">becoming increasingly hostile</a>, thus we are turning back to the physical world for entertainment, and comfort.</p>
            <p>The servicing of those bodies (temperature, food, water, comfort, transport, entertaining stimuli) will be fruitful endeavours.</p>

            <h2>Career Manifestations: New Job! Community Architects</h2>
            <p>Building off of that, those who support the spaces where bodies can come (moderators, administrators, operations leaders/managers) will be in demand. There may also be a gap in the market for a Community Architect, a specialised consultant who enters a private community space and enhances/amplifies it.</p>
            <p>The most successful/ engaged communities are the ones gathering around <b>Documenting the Journey</b>. In a world where AI can produce any result, the process becomes invaluable.</p>

            <h2>Creation Manifestations: handcrafts & #diditshit</h2>
            <p>A rise in Handcrafts including: Craft workshops/apprenticeships pathways, mending clothes/repurposing fabrics, sculpture/pottery/claywork, Braiding</p>
            <p>#diditshit is a trend of just doing the thing - that at least its better than being AI-generated. Also leaning towards a lifestyle trend - the mindset of prioritising the thing, over my satisfaction with the thing.</p>
            <p>Also part of a broader <a href="https://www.youtube.com/watch?v=2XZ9z6OewR0">Art Renaissance</a>.</p>
            <img src="/images/creationDigestion.png" style={{ maxWidth: "300px" }} />
            <p><a href="https://www.instagram.com/p/DQGEf9njT1v/">Source: animamundiherbals via Instagram</a></p>
            
            <h2>Health Manifestations: Gym culture, especially weightlifting</h2>
            <p>Athletics wear but heavier/chunkier than the skin tight athleisure. More of the bulkier, thicker, loose fitting. Also bold colours: vibrant reds, royal blues, and jet blacks.</p>
            
            <h2>Technology Manifestations: Individually, Together</h2>
            <p>A new kind of technology that opens doors to express individuality and connect through those expressions with others. A complete overhaul (or even disavowal) of <a href="https://susam.net/attention-media-vs-social-networks.html">"attention media"</a>, in favour of building of a new kind of market/exchange network.</p>
            <p> Some budding innovations: <span className="red">last update 23/02/2026</span></p>
            <ul>
                <li><a href="https://www.ystrickler.com/start-darkforest/">Dark Forest Operating System</a></li>
                <li><a href="https://discourse.32bit.cafe/">32-Bit Cafe</a></li>
                <li><a href="https://github.com/BilalG1/openslack">open source competitor to Slack</a></li>
                <li><a href="https://matrix.org/"> open source competitor to Discord</a></li>
                <li><a href="https://github.com/KyleKun/one_second_diary">open source competitor One Second Everyday</a></li>
                <li><a href="https://joinloops.org/">open-source competitor to TikTok</a></li>
                <li><a href="https://www.weare8.com/">WeAre8</a></li>
                
            </ul>
            <p className="red">On Feb 9 2026, Discord annouced their <a href="https://discord.com/press-releases/discord-launches-teen-by-default-settings-globally">Teen-by-Default Settings Globally</a>, which stimulated users to consider alternatives.</p>

            <h2 className="red">19 Feb 2026 - Economy Manifestations: Financial Collapse leading to a crackdown on Businesses Becoming Banks</h2>
            <p>The most succesful businesses in this iteration of capitalism are the ones who become banks.</p>
            <p>Every gift card, every points scheme; <a href="https://youtu.be/ggUduBmvQ_4?si=eWmqP96cEEV4IZCD">airlines</a>, <a href="https://youtu.be/7u0_nrsfxXs?si=tOAWFjssni9EBvia">trains</a>, <a href="https://www.afterpay.com/en-AU">Afterpay 'buy now pay later'</a>, and <a href="https://www.apple.com/apple-card/">Apple credit cards</a>.</p>
            <p>Ford is a shining example of the effects of neoliberal policies and disaster capitalism; as after world war 2 their "profitability came to hinge on lending and financial engineering rather than production itself" (Vulture Capitalism, Grace Blakey 2025). The automative manufacturing company that <a href="https://www.factcheck.org/2011/09/ford-motor-co-does-u-turn-on-bailouts/"> received government financial assitance in the aftermath of the 2008 financial crisis</a> and has, as of <a href="https://www.reuters.com/world/fdic-clears-way-ford-general-motors-set-up-industrial-banks-2026-01-22/">Jan 23 2026, gotten FDIC approval to become a 'industrial bank'.</a></p>
            <p>This act is, to me, a flashing neon sign that says "financial collapse imminent". Study history to predict the future: <a href="https://yourbookstore.io/p/vulture-capitalism-corporate-crimes-backdoor-bailouts-and-the-death-of-freedom-2834172">Vulture Capitalism: Corporate crimes, backdoor bailouts, and the death of freedom by Grace Blakeley 2025</a> and <a href="https://yourbookstore.io/p/the-shock-doctrine-the-rise-of-disaster-capitalism">The Shock Doctrine: The rise of Disaster Capitalism by Naomi Klein 2007</a>.</p>
            

            <h2 className="purple">Key Transits</h2>
            <p>All sourced from <a href="https://cafeastrology.com/2025-ephemeris.html">CafeAstrology</a>. These are underpinning my predictions.</p>
            <table>
                <thead>
                    <tr>
                        <th>Planet</th>
                        <th>Sign</th>
                        <th>Start</th>
                        <th>End</th>
                        <th>Analysis + Interpretation</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td>Pluto</td>
                        <td>Aquarius</td>
                        <td>November 2024</td>
                        <td>March 2043</td>
                        <td><a href="https://www.chani.com/blogs/what-you-need-to-know-about-pluto-in-aquarius">Chani</a>:
                        <ul>
                            <li>Between 1777 and 1798 was the American Revolution, the French Revolution, and the beginning of the Industrial Revolution</li>
                            <li>The Age of Enlightenment</li>
                            <li>1792 A Vindication of the Rights of Woman by Mary Wollstonecraft</li>
                        </ul>
                        </td>
                    </tr>
                    <tr>
                        <td>Neptune</td>
                        <td>Aries</td>
                        <td>January 2026</td>
                        <td>March 2038</td>
                        <td><a href="https://www.astrologyfromthesoul.com/post/history-of-neptune-in-aries-1-000-years-in-analysis">Astrology from the Soul</a>:
                        <ul>
                            <li>Humanitarian Actions and Inventions</li>
                            <li>Church Reforms</li>
                            <li>One Ruler</li>
                            <li>The World Splits Apart</li>
                            <li>Coming Full Circle</li>
                            <li>Natural Catastrophes and Epidemics</li>
                        </ul>
                        </td>
                    </tr>
                    <tr>
                        <td>Saturn</td>
                        <td>Aries</td>
                        <td>February 2026</td>
                        <td>April 2028</td>
                        <td><a href="https://www.chani.com/blogs/your-guide-to-saturn-in-aries">Chani</a>:
                        <ul>
                            <li>Personal and collective hardships</li>
                            <li>Build from the rubble of what you've endured</li>
                            <li>From 1996 to 1999 — was the peak of the “dot-com boom”</li>
                        </ul>
                        </td>
                    </tr>
                    <tr>
                        <td>Uranus</td>
                        <td>Gemini</td>
                        <td>April 2026</td>
                        <td>August 2032</td>
                        <td><a href="https://www.chani.com/blogs/your-guide-to-uranus-in-gemini">Chani</a>:
                        <ul>
                            <li>Perceive old problems from fresh angles</li>
                            <li>“Yes, and”</li>
                            <li>Sudden changes, unexpected events, or plot twists</li>
                            <li>1941 to 1949 - WW2, 1944 Bretton Woods Agreement, <a href="https://en.wikipedia.org/wiki/Victory_garden">Victory Gardens</a></li>
                            <li><a href="https://en.wikipedia.org/wiki/1940s#Technology">first electronic digital computing device, </a><a href="https://en.wikipedia.org/wiki/Golden_Age_of_Comic_Books">Golden Age of Comic Books</a>, development of commercial television</li>
                        </ul>
                        </td>
                    </tr>
                    <tr>
                        <td>Chiron</td>
                        <td>Taurus</td>
                        <td>June 2026</td>
                        <td>September 2026</td>
                        <td><a href="https://www.alicesparklykat.com/articles/23/Chiron/">AliceSparklyKat</a>:
                        <ul>
                            <li>Your pain comes from feelings of worthlessness</li>
                            <li>You constantly feel that you have no (social, political, liquid) capital</li>
                            <li>You must define what luxury and joy mean to you</li>
                            <li>1976 - Apple and Microsoft are formed</li>
                        </ul>
                        </td>
                    </tr>
                    <tr>
                        <td>Jupiter</td>
                        <td>Leo</td>
                        <td>July 2026</td>
                        <td>July 2027</td>
                        <td><a href="https://www.alicesparklykat.com/articles/535/The_Philosophy_of_Jupiter_in_Leo/">AliceSparklyKat</a>:
                        <ul>
                            <li>Beliefs that lead you to real world action</li>
                            <li>An idea matters more when it inspires decisive courage</li>
                            <li>Sincerity is self protection</li>
                            <li>Jul 2014 to Aug 2015 - Apple Watch launch, Ex Machina film by Alex Garland, Google became Alphabet, Twitch got acquired by Amazon</li>
                        </ul>
                        </td>
                    </tr>
                </tbody>
            </table>
            <img src="/images/Inner-and-Outer-planets-visual.png" style={{ maxWidth: "800px" }} />
            <p><a href="https://astrologercoach.com/course/chiron-healing-with-astrology/">Source: Astrologer Coach</a></p>
            
            <h2>more resources</h2>
            <ul>
                <li><a href="https://saturnandhoney.substack.com/p/got-a-career-hope-to-have-a-career">Astrology for Business & Career by June from Saturn and Honey</a></li>
                <li><a href="https://www.alicesparklykat.com/articles/697/2026_Horoscopes/">2026 Horoscopes by AliceSparklyKat</a></li>
            </ul>
            
            <div className="red" style={{ paddingBottom: "5px" }}>
                <h1>thank you for reading</h1>
                <button className="red" onClick={() => navigate(-1)}>one more?</button>
            </div>
        </PageLayout>
    );
}

export default Blog_26PS;
